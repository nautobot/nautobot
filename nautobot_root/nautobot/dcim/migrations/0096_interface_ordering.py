...
def _update_model_names(model):
    ...
    for name in model.objects.values_list('name', flat=True).order_by('name').distinct():
        model.objects.filter(name=name).update(_name=nautobot.utilities.ordering.naturalize_interface(name, max_length=100))
...
class Migration(migrations.Migration):
    ...
    operations = [
        ...
        migrations.AddField(
            ...
            name='_name',
            field=nautobot.utilities.fields.NaturalOrderingField('name', blank=True, max_length=100, naturalize_function=nautobot.utilities.ordering.naturalize_interface),
        ),
        migrations.AddField(
