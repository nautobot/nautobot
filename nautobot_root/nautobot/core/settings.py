...
# Set the base directory three levels up
...
if platform.python_version_tuple() < ('3', '6'):
    raise RuntimeError(
        "Nautobot requires Python 3.6 or higher (current: Python {})".format(platform.python_version())
    )
...
STATICFILES_DIRS = (
    os.path.join(BASE_DIR, "nautobot", "project-static"),
)
...
try:
    from nautobot.core import configuration
except ModuleNotFoundError as err:
    if getattr(err, 'name') == 'configuration':
        raise ImproperlyConfigured(
            "Configuration file is not present. Please define nautobot_root/nautobot/core/configuration.py per the documentation."
        )
    raise
...
from nautobot.core.configuration import *  # noqa
...
if 'nautobot.core.authentication.ObjectPermissionBackend' not in AUTHENTICATION_BACKENDS:
    raise ImproperlyConfigured(
