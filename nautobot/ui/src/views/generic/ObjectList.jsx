...
const fetcher = (...urls) => {
  const f = url => fetch(url, { credentials: "include" }).then(r => r.json()).catch(r => r)
...
export default function GenericObjectListView() {
  ...
  const { data = [{}, {}], error, isLoading: isLoadingData } = useSWR(urls, fetcher)
  ...
  return (
    <ObjectListTable tableData={tableData.results} tableHeader={tableFields.data} totalCount={tableData.count} active_page_number={active_page_number} page_size={page_size} />
  )
}
