...
describe("BSTableItem", () => {
  it("renders minus icon when obj is null", () => {
    ...
    const minusIcon = document.evaluate("//svg", document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null);
    expect(minusIcon.singleNodeValue.getAttribute("class")).toBe("svg-inline--fa fa-minus ");
  });
  ...
  it("renders span with className=badge when obj is an array", () => {
    ...
    expect(container.innerHTML).toBe(`<span class="badge" style="background-color: rgb(18, 52, 86);">${obj[0].label}</span>`);
  });
