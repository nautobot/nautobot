...
Navigation menu items provided by Nautobot applications. Each app registers its navbar configuration inside of the `nav_menu` dictionary. Tabs are stored in the top level moving down to groups, items and buttons. Tabs, groups and items can be modified by using the key values inside other application and plugins.
