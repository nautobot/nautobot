...
If your branch modifies a Django model (and as a result requires a database schema modification), after running `invoke makemigrations`, rename the generated migration files to something more meaningful before pushing.

For example `0004_provider_increase_account_length` instead of `0004_auto_20211220_2104`. You’ll also want to run `black` against the generated migration file as the autogenerated code doesn’t follow our style guide by default.
