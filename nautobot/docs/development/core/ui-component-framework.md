...
[`Tab`](#tabs), [`Panel`](#panels), and [`Button`](#buttons) all share a common ancestor, the `Component` class. Every `Component` must declare its `weight` (which defines the relative order of related components, and allows for Apps to potentially define [extensions](../apps/api/ui-extensions/object-views.md) that are interleaved with the base components built in to the baseline Nautobot view). A `Component` may also declare a list of `required_permissions` (if there are additional required permissions whose absence should prevent the component from even being displayed at all), and any number of additional keyword args specific to the component subclass in use. Each `Component` also automatically receives a `component_id`, which is a hash of the component's class and instance attributes used to uniquely identify any given `Component` within a given `ObjectDetailContent`. (In the highly unlikely case where two `Components` for the same view receive identical autogenerated `component_id` hashes, you do have the _option_ to explicitly define a manual `component_id` for either or both of them.)

#### Deferred Rendering with HTMX

+++ 3.1.0 "Added support for deferred rendering of individual components"

This feature allows for components, that require expensive calculations or access to remote systems to be populated, to be included appropriately in the page, without delaying the initial loading of the page and negatively affecting its responsiveness to user input.

Each `Component` has a `deferred_render` attribute, defaulting to `False`. If this flag is changed to `True` for a given component (most commonly a `Panel`, although `Button` components _do_ support deferred rendering as well), this component will **not** be rendered fully on the initial page render of the relevant view; instead, only its `placeholder_template_path` template will be rendered with the baseline view render context. (In other words, the component-specific `get_extra_context()` will **not** be called in this case.) The `placeholder_template_path` template is responsible for defining appropriate HTMX attributes to ensure that the receiving browser will subsequently make an appropriate HTMX request to render and inject the now-fully-rendered component into the page.

A basic example is the generic placeholder template `nautobot/core/templates/components/htmx/component_placeholder.html`:

```html
<div id="component-{{ component.component_id }}"
     hx-get=""
     hx-select="#component-{{ component.component_id }}"
     hx-trigger="load"
     hx-vals='{"component_id": "{{ component.component_id }}"}'
>
...
This placeholder template renders a Bootstrap "spinner" element in place of the component, and instructs HTMX to make an immediate followup query (`hx-trigger="load")` to the same URL as the initial page was rendered (`hx-get=""`), but with the HTMX `HX-Request: true` header included and the additional query parameter `?component_id=<this component's component_id>` (`hx-vals=...`). `NautobotUIViewSet` and the older generic `ObjectView` both know how to handle such a modified query, by retrieving and rendering only the requested component. When the browser receives this rendered component, it selects the component's contents (`hx-select=...`) and swaps them into the page in place of the placeholder.

Depending on the specific component, a more specific `placeholder_template_path` may be desirable to render a less-generic placeholder - examples included with Nautobot core include `nautobot/core/templates/components/panel/panel_placeholder.html` and `nautobot/core/templates/components/button/button_placeholder.html`.
...
| `row_overviews_visibility` | No | `TABLE_DEFAULT` | Include a per-row button that expands the row to reveal the object's overview |
