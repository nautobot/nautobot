...
!!! note
    `NavMenuAddButton` and `NavMenuImportButton` are subclasses of `NavMenuButton` that can be used to provide the commonly used "Add" and "Import" buttons which provides the respective expected defaults for `title`, `icon_class`, `button_class`, and `weight`.
