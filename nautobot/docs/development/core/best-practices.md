...
- FilterSets **must** publish all user-visible model fields from a model, including related fields.
...
- In general, if it is necessary to explicitly declare a model-field filter (except for foreign-key and similar related-object filters, see below) you **must** use one of Nautobot's `MultiValue<type>Filter` classes, for example `MultiValueCharFilter` for a `CharField`, **not** one of `django_filters`'s baseline `<type>Filter` classes (for example, `CharFilter`). This matches the pattern used throughout Nautobot when autogenerating such filters.
    - One minor exception: there is no `MultiValueBooleanFilter` at this time, so boolean fields may use a simple `BooleanFilter` if needed.
    - Similarly, use `nautobot.core.filters.MultipleChoiceFilter` and `nautobot.core.filters.ModelMultipleChoiceFilter` in preference to the `django_filters` classes of the same names. Besides other Nautobot-specific behavior, these classes derive their `distinct` flag automatically (see below), while the `django_filters` versions do not.

```python
class LocationFilterSet(NautobotFilterSet):
    # Incorrect - wrong filter class
    name = CharFilter()

    # Correct, though redundant with the autogenerated filter for this field when using `fields="__all__"`
    name = MultiValueCharFilter()

    # Correct - `distinct=True` is derived automatically here, as this traverses a to-many relation
    devices__name = MultiValueCharFilter()
```

- In Nautobot 2.0 and later, for all foreign-key related fields and their corresponding reverse-relations:
    - If there is no appropriate single field that could be used as a natural key (e.g. a globally-unique `name` or `slug`), then the default filtering behavior for this field (using `nautobot.core.filters.ModelMultipleChoiceFilter`) can be used for now, until [issue 2875](https://github.com/nautobot/nautobot/issues/2875) is implemented to allow for the use of multiple fields with `NaturalKeyOrPKMultipleChoiceFilter`.
    - Otherwise, the field **must** be shadowed with a Nautobot `NaturalKeyOrPKMultipleChoiceFilter` which will automatically try to lookup by UUID or `name` depending on the value of the incoming argument (e.g. UUID string vs. name string).
        - This provides an advantage over the default `nautobot.core.filters.ModelMultipleChoiceFilter` which only supports a UUID (`pk`) value as an input.
...
- A queryset [`.distinct()`](https://django-filter.readthedocs.io/en/stable/ref/filters.html#distinct) call is required if and only if a filter traverses a to-many relation -- a many-to-many field, a reverse foreign key, or a generic relation. Without it, such a filter returns duplicated results; with it, any other filter pays a database cost at scale for no benefit.

    You **should not** normally declare `distinct` at all. Nautobot's `MultiValue<type>Filter`, `MultipleChoiceFilter`, and `ModelMultipleChoiceFilter` classes -- and everything derived from them, such as `NaturalKeyOrPKMultipleChoiceFilter` and `TreeNodeMultipleChoiceFilter` -- inspect their own `field_name` against the filterset's model and derive the correct value themselves. `SearchFilter` does the same across all of its `filter_predicates`, applying `.distinct()` only if at least one predicate traverses a to-many relation. `FilterTestCases.FilterTestCase.test_filters_distinct` asserts this for every filter in the filterset under test, so any FilterSet with a `FilterTestCase` (in Nautobot or in an App) is checked automatically.

    Declare `distinct` explicitly **only** where the derived value would be wrong, which in practice means only where `field_name` is not a real model field path. `RelationshipFilter` is the one example in Nautobot core, as its `field_name` is a synthetic `cr_<key>__<side>` string. Add a comment explaining why.

    Note that `distinct` has no effect at all on a filter that specifies a `method`, as `django-filter` replaces such a filter's `filter()` method entirely; a filter method is responsible for calling `.distinct()` itself where needed.
