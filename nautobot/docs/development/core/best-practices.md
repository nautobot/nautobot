...
- In general, if it is necessary to explicitly declare a model-field filter (except for foreign-key and similar related-object filters, see below) you **must** use one of Nautobot's `MultiValue<type>Filter` classes, for example `MultiValueCharFilter` for a `CharField`, **not** one of `django_filters`'s baseline `<type>Filter` classes (for example, `CharFilter`). This matches the pattern used throughout Nautobot when autogenerating such filters.
    ...
    - Important: for historical reasons, `django_filters.MultipleChoiceFilter` (which is the ancestor class for all of Nautobot's `MultiValue<type>Filter` classes) defaults to `distinct=True` if not explicitly declared otherwise. For most non-related-object filters, this adds unnecessary overhead when querying the database to apply such a filter, and so you should strongly consider explicitly declaring `distinct=False` on such filters, unless there is a clear need (such as filtering on a field reached through a many-to-many relation) to apply `distinct()` to the queryset, in which case we recommend declaring `distinct=True` to make this intent explicit.
...
class LocationFilterSet(NautobotFilterSet):
    ...
    # Incorrect - unnecessary performance overhead
    name = MultiValueCharFilter()

    # Correct, though redundant with autogenerated filter for this field when using `fields="__all__"`
    name = MultiValueCharFilter(distinct=False)

    # Correct, due to traversing a to-many relation
    devices__name = MultiValueCharFilter(distinct=True)
```
...
- In Nautobot 2.0 and later, for all foreign-key related fields and their corresponding reverse-relations:
    ...
    - As noted above, these filters also default to `distinct=True` when declared, so in many cases you will want to explicitly declare `distinct=False` to improve performance.
...
- Filters **must** be declared using [`distinct=True`](https://django-filter.readthedocs.io/en/stable/ref/filters.html#distinct) if a queryset `.distinct()` is required to be called on the queryset to avoid returning duplicated results. Conversely, for filters that *default* to `distinct=True` (which is true for most of Nautobot's filter classes!), you **should** declare `distinct=False` if no such `.distinct()` is required, because `.distinct()` can be costly at scale.
