...
+/- 2.2.7 "Hashing of migrations in the factory dump"
    The test runner now calculates the SHA1 hash of applied database migrations and uses that as a key when creating/locating the factory data file. This serves as a way to avoid inadvertently using cached test data from the wrong branch or wrong set of migrations, and reduces the frequency with which you might need to manually delete the fixture file. For example, the set of migrations present in `develop` might result in a `factory_dump.72b71317c5f5c047493eaff8a04e35dcb7f6b18c.json`, while those in `next` might result in `factory_dump.f8164d3b07c771ef2a803c82e8dd24ae153a7f13.json` - both files can coexist, and when you switch between branches during development, the correct one will automatically be selected.
