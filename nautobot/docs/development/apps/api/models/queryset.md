...
A [**QuerySet**](https://docs.djangoproject.com/en/stable/ref/models/querysets/) is a core Django concept representing a collection of model records described by a given database query. QuerySets are constructed using the model's manager (e.g., `Device.objects`) and provide methods such as `.filter()`, `.exclude()`, and `.order_by()` to build up queries. QuerySets are Nautobot's _programmatic_ (Python) API for interacting with the database, and as such, are commonly used within Jobs and Nautobot Apps, but are _not_ generally used directly by end users accessing Nautobot via the UI, REST API, or GraphQL interfaces.
...
FilterSets are not a replacement for QuerySets; rather, they operate on top of QuerySets, at the _user interaction_ layer (i.e. the UI, REST API, and GraphQL interfaces) of Nautobot. When any of these interfaces receives filter parameters, those parameters pass through the corresponding FilterSet and are programmatically translated to one or more QuerySet queries.

FilterSets allow for Nautobot to both _extend_ and _restrict_ the kinds of database queries that can be executed by a user. For example, a FilterSet can provide a single filter parameter which corresponds to a combination of multiple distinct database queries, which may not be easily expressed as a single QuerySet. Conversely, some queries that may be possible via a QuerySet may be specifically excluded from the FilterSet built atop it; for example, Nautobot does _not_ provide a FilterSet filter allowing users to look up Users by their `password`, for obvious reasons.
