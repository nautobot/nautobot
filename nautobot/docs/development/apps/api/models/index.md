...
+++ 3.2.0 "Staff-only change log records"
    A model can opt into having its `ObjectChange` records hidden from non-staff viewers by declaring the class attribute `is_staff_only_changelog_model = True`. This is intended for models whose change history should not be visible to ordinary users (e.g. `User`, `Token`, `ObjectPermission`).
