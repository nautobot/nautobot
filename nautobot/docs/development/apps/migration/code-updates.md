...
Most changes in code location arise from the merging of the `nautobot.utilities` module into the `nautobot.core` module. Use the below table to understand where you code may have originally been and the table after to understand where to properly import from `nautobot.apps`.
...
All imported (except for models) should be imported from `nautobot.apps.*` as described here.
