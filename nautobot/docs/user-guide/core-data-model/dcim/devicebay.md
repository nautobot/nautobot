...
A child device can also be a parent-child device type, so device bays can be nested across multiple levels. For example, a blade chassis might contain a modular compute sled that itself holds multiple microserver nodes, each modeled as a child device.
