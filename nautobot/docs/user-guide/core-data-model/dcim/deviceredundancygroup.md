...
=== "HA pairs"
    ...
    # Set the sync address (HA self-IP)
    modify cm device {{ device.name }} { configsync-ip {{ device.primary_ip4.address.split("/")[0] }} }
    # Add the peer to the device trust
    run cm add-to-trust wire-address {{ peer.primary_ip4.address.split("/")[0] }} user admin

    # Create the sync-failover device group
    create cm device-group {{ group.name }} { devices { {{ group.devices | map(attribute="name") | join(" ") }} } type sync-failover }
    {% endfor %}
    ```
