...
+++ 3.2.0
    !!! tip "Individual DHCP addresses vs. DHCP ranges"
        The `DHCP` IP Address `type` and an [IP Address Range](ipaddressrange.md) with a DHCP role address different needs. Use the `DHCP` **type on an individual IP Address** when you want to document a specific address that is (or will be) handed out by DHCP. For example, a DHCP reservation tied to a particular device. Use an **IP Address Range** when you want to represent the DHCP scope as a whole (for example `10.0.0.100–10.0.0.200`) without recording each address individually. The two can be combined: a non-exclusive IP Address Range can represent the scope while individual IP Addresses document specific reservations within it.
