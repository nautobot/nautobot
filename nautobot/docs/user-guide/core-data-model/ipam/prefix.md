...
### Pool Prefixes vs. IP Ranges

+++ 3.2.0
    A "Pool" Prefix and an [IP Range](iprange.md) can both represent a span of addresses used as a unit, such as a DHCP scope or NAT pool, and their roles partially overlap. The key difference is alignment:
    ...
    * An `IP Range` is defined by an arbitrary start and end address (for example `10.0.0.50–10.0.0.200`) and does **not** need to align to a CIDR boundary. Use an IP Range when the span you want to represent is not a clean subnet.
