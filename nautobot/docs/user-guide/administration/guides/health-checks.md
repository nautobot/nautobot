...
!!! warning
    If you are using Celery version `5.6.1` breaks the creation of Worker's heartbeat file, but still generate readiness files as expected. Upgrading to Celery `5.6.2` resolves the issue.
