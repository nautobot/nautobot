...
Ensure that the `NAUTOBOT_ROOT` permissions are set to `0755`, allowing other members of the `nautobot` user group (including the `nginx` account) to read and execute files in this directory:

```no-highlight title="Update access permissions to 0755 for $NAUTOBOT_ROOT"
chmod 0755 $NAUT
#### Restart NGINX
