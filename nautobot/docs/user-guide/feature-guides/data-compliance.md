...
!!! note
    ...
    For example, if a user fixes a non-compliant attribute on an object and checks the Data Compliance tab, the object will still appear as invalid for that built-in rule. This invalid status won't update until the job is rerun with the `Run existing validation rules?` option enabled.
