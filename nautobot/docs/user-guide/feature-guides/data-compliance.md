...
!!! note
    ...
    However, `DataCompliance` objects generated by the `Run existing validation rules?` option in the job do not update or re-validate automatically. They will only refresh if the job is explicitly run again.
    ...
    When the user manually runs the `RunRegisteredDataComplianceRules` job with the `Run existing validation rules?` option enabled, the system identifies any `Location` objects that have an `asn` value outside the allowed range. It then generates `Data Compliance` objects to flag these violations.
    If the user corrects the invalid `asn` values in the `Location` objects, the previously generated `Data Compliance` objects will not be automatically updated to reflect the fixes. To mark them as valid, the user must manually re-run the `RunRegisteredDataComplianceRules` job again with the `Run existing validation rules?` option enabled.
...
If you also want to generate compliance results for built-in **and** user-created validation rules, check the `Run existing validation rules?` option. This will apply all validation rules, regardless of whether they are currently **enabled** or **disabled** for the objects assigned to them.
