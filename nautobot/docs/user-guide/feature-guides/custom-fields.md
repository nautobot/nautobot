...
+/- 2.0.0
    Custom field model no longer has a `name` attribute in v2.0. Instead, its pre-existing `label` attribute (unique for all Custom field instances) replaced `name` to be used exclusively for display purposes in the UI.
    Custom field model's `slug` attribute was renamed to `key` (unique for all Custom Field instances), which contains a GraphQL-safe string used as the key in the underlying custom field data dictionary.
