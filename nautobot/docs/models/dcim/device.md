...
Each device must be assigned a site, device role, and operational [`status`](../../../models/extras/status/), and may optionally be assigned to a specific location and/or rack within a site. A platform, serial number, and asset tag may optionally be assigned to each device.
