...
Each IP address can also be assigned an operational status and a functional role. Default status options are provided, including the following, but may be customized as with any other "status" field in Nautobot (more information on custom status model [`here.`](https://nautobot.readthedocs.io/en/latest/models/extras/status/)):
