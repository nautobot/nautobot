...
!!! note
    GitHub Apps are not currently supported. GitHub Apps are not specifically focused on the workflow of creating/updating/deleting files. They can indeed perform these actions, but in order to accomplish that the client side MUST hit GitHub APIs instead of using standard command line or Python Git library tooling.
