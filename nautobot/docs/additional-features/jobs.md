...
!!! note
    Using `self.log_failure()`, in addition to recording a log message, will flag the overall job as failed, but it will **not** stop the execution of the job. To end a job early, you can use a Python `raise` or `return` as appropriate. If the job made changes, it is best to raise `nautobot.utilities.exceptions.AbortTransaction` to ensure any database changes are rolled back as part of the process of ending the job.
