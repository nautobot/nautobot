...
!!! warning
    A Job **must** be [enabled](./jobs.md#enabling-jobs-for-running) and cannot have [has_sensitive_variables](./jobs.md#hassensitivevariables) set to `True` in order to be scheduled. A warning banner will appear on the run Job view with the reason Job Scheduling is not an option.
