...
...
    "relationships": {
        "location-to-vrf": {
            ...
            "name": "Single Location to Single VRF",
            "type": "one-to-one",
            "destination": {
                "label": "VRF",
...
In the example above we can see that a single VRF, `green`, is a destination for the `location-to-vrf` Relationship from this Location, while there are currently no VRFs associated as sources for the `vrfs-to-locations` Relationship to this Location.
...
!!! Note
    Relationship keys can be omitted from the `"relationships"` dictionary, in which case the associations for that Relationship will be left unmodified. In the second example above, the existing association for the `"location_to_vrf"` Relationship would be replaced, but the `"vrfs_to_locations"` Relationship's associations would remain as-is.
...
Note that there is no requirement for the attributes to be identical among objects. For instance, it's possible to update the status of one location along with the name of another in the same request.
