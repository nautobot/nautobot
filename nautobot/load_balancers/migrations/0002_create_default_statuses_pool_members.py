...
from nautobot.load_balancers.choices import LoadBalancerPoolMemberStatusChoices
...
def populate_load_balancer_pool_member_status_choices(apps, schema_editor):
    ...
    content_type = ContentType.objects.get_for_model(apps.get_model("load_balancers", "LoadBalancerPoolMember"))
    for choice_kwargs in choices:
        ...
        try:
            ...
            raise SystemExit(
                f"Unexpected error while creating status {name} for load_balancers.LoadBalancerPoolMember: {err}"
            ) from err
...
def clear_load_balancer_pool_member_status_choices(apps, schema_editor):
    ...
    for name in statuses:
        try:
            ...
            raise SystemExit(
                f"Unexpected error while removing load_balancers.LoadBalancerPoolMember from status {name}: {err}"
            ) from err
