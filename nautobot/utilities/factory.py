...
def random_instance(model, **kwargs):
    """
    ...
    return factory.LazyFunction(lambda: factory.random.randgen.choice(queryset.all()) if queryset.count() else None)


def get_random_instances(model, minimum=0, maximum=None, **kwargs):
    """
    ...
    if branch == 1 or count == 1 or maximum == 1:
        ...
        return [random_instance(model, **kwargs).evaluate(None, None, {"locale": None})]
