...
DEFAULT_TITLES: dict[str, str] = {
    "list_action": "{{ verbose_name_plural|bettertitle }}",
    "retrieve_action": "{{ object.display|default:object }}",
    "destroy_action": "Delete {{ verbose_name }}?",
    "create_action": "Add a new {{ verbose_name }}",
    "update_action": "Editing {{ verbose_name }} {{ object.display|default:object }}",
    "bulk_destroy_action": "Delete {{ total_objs_to_delete }} {{ verbose_name_plural|bettertitle }}?",
    "bulk_rename_action": "Renaming {{ selected_objects|length }} {{ verbose_name_plural|bettertitle }} on {{ parent_name }}",
    "bulk_update_action": "Editing {{ objs_count }} {{ verbose_name_plural|bettertitle }}",
    "changelog_action": "{{ object.display|default:object }} - Change Log",
    "notes_action": "{{ object.display|default:object }} - Notes",
    "approve_action": "Approve {{ verbose_name|bettertitle }}?",
    "deny_action": "Deny {{ verbose_name|bettertitle }}?",
}
...
class PageHeadings(Titles):
    ...
    def __init__(self, list_action=None, **kwargs):
        default_list_action = "{% format_title_with_saved_view verbose_name_plural|bettertitle %}"
