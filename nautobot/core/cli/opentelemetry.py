...
def instrument():
    ...
    resource = Resource(
        attributes={SERVICE_NAME: "nautobot", SERVICE_VERSION: __version__}
    )
    ...
    if "otlp" in settings.OTEL_TRACES_EXPORTER:
        ...
        trace.get_tracer_provider().add_span_processor(
            BatchSpanProcessor(OTLPSpanExporter(**otlp_settings))
        )
    ...
    if settings.OTEL_METRICS_EXPORTER:
        ...
        if "otlp" in settings.OTEL_METRICS_EXPORTER:
            ...
            if settings.OTEL_EXPORTER_OTLP_PROTOCOL == "http":
                ...
                otlp_settings["insecure"] = settings.OTEL_EXPORTER_OTLP_INSECURE
            readers.append(
                PeriodicExportingMetricReader(OTLPMetricExporter(**otlp_settings))
            )
    ...
    DjangoInstrumentor().instrument(
        tracer_provider=provider, is_sql_commentor_enabled=True
    )
