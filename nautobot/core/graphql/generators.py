...
def generate_fk_resolver(field_name, resolver_name, related_model):
    ...
    `nautobot.core.graphql.types.OptimizedNautobotObjectType`). This resolver returns the related object
    (already loaded via `select_related` by the optimizer) only if the requesting user is permitted to view
    it, and `None` otherwise. The permission check reuses the per-request permitted-PK cache, so it does not
