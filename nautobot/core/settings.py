...
NAUTOBOT_RATE_LIMITING_SETTINGS = {}
if "NAUTOBOT_RATE_LIMITING_MODE" in os.environ:
    NAUTOBOT_RATE_LIMITING_SETTINGS["MODE"] = float(os.getenv("NAUTOBOT_RATE_LIMITING_MODE"))
if "NAUTOBOT_RATE_LIMITING_WINDOW_SECONDS" in os.environ:
    NAUTOBOT_RATE_LIMITING_SETTINGS["WINDOW_SECONDS"] = float(os.getenv("NAUTOBOT_RATE_LIMITING_WINDOW_SECONDS"))
if "NAUTOBOT_RATE_LIMITING_RECORDS_LIMIT" in os.environ:
    NAUTOBOT_RATE_LIMITING_SETTINGS["LIMIT"] = float(os.getenv("NAUTOBOT_RATE_LIMITING_RECORDS_LIMIT"))
if "NAUTOBOT_RATE_LIMITING_HEURISTIC_WEIGHTS" in os.environ:
    NAUTOBOT_RATE_LIMITING_SETTINGS["HEURISTIC_WEIGHTS"] = float(os.getenv("NAUTOBOT_RATE_LIMITING_HEURISTIC_WEIGHTS"))
if "NAUTOBOT_RATE_LIMITING_CALIBRATION_LOG" in os.environ:
    NAUTOBOT_RATE_LIMITING_SETTINGS["CALIBRATION_LOG"] = float(os.getenv("NAUTOBOT_RATE_LIMITING_CALIBRATION_LOG"))
if "NAUTOBOT_RATE_LIMITING_CALIBRATION_SAMPLE_RATE" in os.environ:
    NAUTOBOT_RATE_LIMITING_SETTINGS["CALIBRATION_SAMPLE_RATE"] = float(os.getenv("NAUTOBOT_RATE_LIMITING_CALIBRATION_SAMPLE_RATE"))

NAUTOBOT_REST_RATE_LIMITING_DEFAULT_SETTINGS = {}
if "NAUTOBOT_RATE_LIMITING_READ_COST" in os.environ:
    NAUTOBOT_REST_RATE_LIMITING_DEFAULT_SETTINGS["read_cost"] = float(os.getenv("NAUTOBOT_RATE_LIMITING_READ_COST"))
if "NAUTOBOT_RATE_LIMITING_WRITE_COST" in os.environ:
    NAUTOBOT_REST_RATE_LIMITING_DEFAULT_SETTINGS["write_cost"] = float(os.getenv("NAUTOBOT_RATE_LIMITING_WRITE_COST"))
if "NAUTOBOT_RATE_LIMITING_RECORDS_PER_PAGE_DIVISOR" in os.environ:
    NAUTOBOT_REST_RATE_LIMITING_DEFAULT_SETTINGS["records_per_page_divisor"] = float(os.getenv("NAUTOBOT_RATE_LIMITING_RECORDS_PER_PAGE_DIVISOR"))
if "NAUTOBOT_RATE_LIMITING_PER_JOIN_COST" in os.environ:
    NAUTOBOT_REST_RATE_LIMITING_DEFAULT_SETTINGS["per_join_cost"] = float(os.getenv("NAUTOBOT_RATE_LIMITING_PER_JOIN_COST"))
if "NAUTOBOT_RATE_LIMITING_UNINDEXABLE_LOOKUP_COST" in os.environ:
    NAUTOBOT_REST_RATE_LIMITING_DEFAULT_SETTINGS["unindexable_lookup_cost"] = float(os.getenv("NAUTOBOT_RATE_LIMITING_UNINDEXABLE_LOOKUP_COST"))
if "NAUTOBOT_RATE_LIMITING_DEPTH_MULTIPLIER_PER_LEVEL" in os.environ:
    NAUTOBOT_REST_RATE_LIMITING_DEFAULT_SETTINGS["depth_multiplier_per_level"] = float(os.getenv("NAUTOBOT_RATE_LIMITING_DEPTH_MULTIPLIER_PER_LEVEL"))
if "NAUTOBOT_RATE_LIMITING_COMPUTED_FIELDS_MULTIPLIER" in os.environ:
    NAUTOBOT_REST_RATE_LIMITING_DEFAULT_SETTINGS["computed_fields_multiplier"] = float(os.getenv("NAUTOBOT_RATE_LIMITING_COMPUTED_FIELDS_MULTIPLIER"))
if "NAUTOBOT_RATE_LIMITING_CSV_MULTIPILER" in os.environ:
    NAUTOBOT_REST_RATE_LIMITING_DEFAULT_SETTINGS["csv_multiplier"] = float(os.getenv("NAUTOBOT_RATE_LIMITING_CSV_MULTIPILER"))
...
if TESTING:
    ...
    # Default (OpenTelemetry log correlation OFF) shape. When correlation is enabled, the trace/span
    # IDs are surfaced in this output by `nautobot.core.logging.enable_otel_log_correlation()`, called
    # from `nautobot.core.cli._preprocess_settings` after the config is fully loaded. That post-load
    # step (not this block) is what appends the ` [trace_id=... span_id=...]` suffix to the formatters
    # and attaches `OtelTraceContextFilter` to the handlers -- doing it there rather than here lets it
    # read the *resolved* OTEL_PYTHON_DJANGO_INSTRUMENT / OTEL_PYTHON_LOG_CORRELATION settings (which
    # are defined later in this module and may be overridden in nautobot_config.py), instead of
    # re-reading the raw env vars at settings-import time.
    LOGGING = {
        "version": 1,
        "disable_existing_loggers": False,
        "filters": {
