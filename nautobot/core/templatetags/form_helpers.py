...
def render_form(form, skip_excluded_fields=True):
    """
    Render an entire form from template and skip over `tags` and `object_note` fields.
    Setting `skip_excluded_fields` to True prevents skipping over `tags` and `object_note` fields
    ...
    return {
        "form": form,
        "skip_excluded_fields": skip_excluded_fields,
    }
