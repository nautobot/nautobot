...
class ObjectListViewTitlesTest(TestCase):
    ...
    @override_settings(ALLOWED_HOSTS=["*"], PAGINATE_COUNT=5, MAX_PAGE_SIZE=10)
    def test_renderer_content_type_uses_model_policy_when_view_has_no_override(self):
        """Renderer should use model.for_concrete_model when the view has no explicit override."""
        path = reverse("cloud:cloudresourcetype_list")
        request = self.factory.get(path)
        request.user = self.user
