...
class NautobotTestRunner(DiscoverRunner):
    ...
    `nautobot.utilities.testing.integration.SplinterTestCase` has this tag, therefore any test cases
    inheriting from that class do not need to be explicitly tagged.

    Only integration tests that DO NOT inherit from `SplinterTestCase` will need to be explicitly tagged.
    """
