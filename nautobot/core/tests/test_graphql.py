...
from nautobot.core.graphql.utils import PERMISSION_ENFORCED_RESOLVER_ATTR, str_to_var_name
...
def rebuild_graphql_schema():
    ...
    """
    query_type = type("Query", (ObjectType, generate_query_mixin()), {})
...
def execute_query_on_rebuilt_schema(query, variables=None, request=None, user=None):
    ...
    Either `request` or `user` must be provided. Mirrors `nautobot.core.graphql.execute_query`, but runs
    against a schema rebuilt at call time so that Relationships / Custom Fields created during the test are
    present.
    """
...
class GraphQLRelatedObjectPermissionTest(GraphQLTestCaseBase):
    ...
    def test_all_related_object_fields_are_permission_enforced(self):
        ...
        This scans the whole schema (via introspection) and, for every field whose output type is -- or, for
        unions, may be -- a restrictable model, asserts the field's resolver is marked as permission-enforcing
        (see `nautobot.core.graphql.utils.mark_permission_enforced`, set by `permission_safe_resolver` /
        `permission_safe_attribute_resolver` and the auto-generated FK / filter / relationship resolvers). A
        newly-added accessor that returns related objects without enforcement is caught here rather than
        shipping as a permission leak.
        ...
        introspection_types = rebuild_graphql_schema().introspect()["__schema"]["types"]
        types_by_name = {entry["name"]: entry for entry in introspection_types}
        registry_by_name = {schema_type._meta.name: schema_type for schema_type in registry["graphql_types"].values()}

        def restrictable_labels(named_type_name):
            """Restrictable model labels a named GraphQL type (object, or the members of a union) can yield."""
            entry = types_by_name.get(named_type_name, {})
            if entry.get("kind") == "UNION":
                member_names = [member["name"] for member in (entry.get("possibleTypes") or [])]
            else:
                member_names = [named_type_name]
            labels = set()
            for member_name in member_names:
                schema_type = registry_by_name.get(member_name)
                if schema_type is None:
                    continue
                manager = getattr(schema_type._meta.model, "_default_manager", None)
                if manager is not None and hasattr(manager, "restrict"):
                    labels.add(schema_type._meta.model._meta.label_lower)
            return labels

        def unwrap(type_ref):
            while type_ref and type_ref.get("ofType"):  # unwrap NON_NULL / LIST wrappers
                type_ref = type_ref["ofType"]
            return type_ref or {}

        unenforced = []
        for entry in introspection_types:
            if entry.get("kind") != "OBJECT":
                continue
            schema_type = registry_by_name.get(entry["name"])
            if schema_type is None:
                continue  # skip Query root, introspection types, and non-model types
            for field in entry.get("fields") or []:
                named_type_name = unwrap(field["type"]).get("name")
                if not named_type_name or not restrictable_labels(named_type_name):
                    continue
                resolver = getattr(schema_type, f"resolve_{field['name']}", None)
                if resolver is not None and getattr(resolver, PERMISSION_ENFORCED_RESOLVER_ATTR, False):
                    continue
                if (entry["name"], field["name"]) in allowed_unenforced:
                    continue
                unenforced.append(f"{entry['name']}.{field['name']} -> {sorted(restrictable_labels(named_type_name))}")
