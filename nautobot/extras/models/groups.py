...
class DynamicGroup(PrimaryModel):
    ...
    def _set_members(self, value):
        ...
        if isinstance(value, models.QuerySet):
            ...
            # Removing any existing `order_by()` on `members` and `value` querysets is needed to avoid an error on sqlite
            to_remove = self.members.order_by().only("id").difference(value.order_by().only("id"))
            self._remove_members(to_remove)
            to_add = value.order_by().only("id").difference(self.members.order_by().only("id"))
    ...
    def add_members(self, objects_to_add):
        ...
        if isinstance(objects_to_add, models.QuerySet):
            if objects_to_add.model != self.model:
                raise TypeError(f"QuerySet does not contain {self.model._meta.label_lower} objects")
            objects_to_add = objects_to_add.order_by().only("id").difference(self.members.order_by().only("id"))
        else:
            for obj in objects_to_add:
                if not isinstance(obj, self.model):
                    raise TypeError(f"{obj} is not a {self.model._meta.label_lower}")
            existing_members = self.members
            objects_to_add = [obj for obj in objects_to_add if obj not in existing_members.order_by()]
