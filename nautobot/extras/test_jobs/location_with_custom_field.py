...
class TestCreateLocationWithCustomField(Job):
    ...
    def run(self):
        with transaction.atomic():
            ...
            location_1 = Location.objects.create(name="Test Location", location_type=location_type, status=status)
            location_1.cf[cf.key] = "some-value"
            location_1.save()
            logger.info("Created a new location", extra={"object": location_1})

            location_2 = Location.objects.create(name="Test Site Two", location_type=location_type, status=status)
