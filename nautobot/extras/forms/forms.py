...
class JobEditForm(NautobotModelForm):
    ...
    def clean(self):
        ...
        if cleaned_data["sensitive"] is True and cleaned_data["approval_required"] is True:
            raise ValidationError({"approval_required": "A sensitive job cannot be marked for approval"})
        return cleaned_data
...
class JobFilterForm(BootstrapMixin, forms.Form):
    ...
    sensitive = forms.NullBooleanField(required=False, widget=StaticSelect2(choices=BOOLEAN_WITH_BLANK_CHOICES))
