...
def update_custom_field_choice_data(field_id, old_value, new_value):
    ...
    if field.type == CustomFieldTypeChoices.TYPE_SELECT:
        ...
        for ct in field.content_types.all():
            ...
            for obj in model.objects.filter(**{f"_custom_field_data__{field.slug}": old_value}):
                obj._custom_field_data[field.slug] = new_value
                obj.save()
        ...
        for ct in field.content_types.all():
            model = ct.model_class()
            for obj in model.objects.filter(**{f"_custom_field_data__{field.slug}__contains": old_value}):
                old_list = obj._custom_field_data[field.slug]
                new_list = [new_value if e == old_value else e for e in old_list]
                obj._custom_field_data[field.slug] = new_list
                obj.save()

    else:
        logger.error(f"Unknown field type, failing to act on choice data for this field {field.slug}.")
        return False
...
@nautobot_task
def delete_custom_field_data(field_slug, content_type_pk_set):
    """
    Delete the values for a custom field

    Args:
        field_slug (str): The slug of the custom field which is being deleted
    ...
    with transaction.atomic():
        for ct in ContentType.objects.filter(pk__in=content_type_pk_set):
            model = ct.model_class()
            for obj in model.objects.filter(**{f"_custom_field_data__{field_slug}__isnull": False}):
                del obj._custom_field_data[field_slug]
                obj.save()
    ...
    with transaction.atomic():
        for ct in ContentType.objects.filter(pk__in=content_type_pk_set):
            ...
            for obj in model.objects.all():
                obj._custom_field_data.setdefault(field.slug, field.default)
                obj.save()
