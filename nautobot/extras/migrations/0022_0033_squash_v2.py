...
def _create_job_model(job_model_class, class_path):
    try:
        ...
        if len(source) > 110:
            print(
                f'Skipping Job model creation from "{class_path}" as the source is too long'
            )
            return None
        if len(module_name) > 100:
            print(
                f'Skipping Job model creation from "{class_path}" as the module_name is too long'
            )
            return None
        if len(job_class_name) > 100:
            print(
                f'Skipping Job model creation from "{class_path}" as the job_class_name is too long'
            )
            return None
        ...
        if created:
            print(
                f'Created Job model "{module_name}: {job_class_name}" for {class_path}'
            )
        return job_model
    except ValueError:
...
def migrate_job_git_repository_data(apps, schema_editor):
    ...
    for job in Job.objects.filter(source__startswith="git."):
        ...
        try:
            ...
            print(
                f'Git repository "{repo_slug}" not found for Job {job.module_name} {job.job_class_name}!'
            )
        job.source = "git"
        job.save()
...
class Migration(migrations.Migration):
    ...
    operations = [
        ...
        migrations.RunPython(
            migrate_job_git_repository_data, reverse_migrate_job_git_repository_data
        ),
        migrations.CreateModel(
            name="DynamicGroup",
            fields=[
