...
class JobFileUploadTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.example_file = SimpleUploadedFile(name="example.txt", content=cls.file_contents)
        cls.job_content_type = ContentType.objects.get(app_label="extras", model="job")

    def setUp(self):
        super().setUp()
        self.example_file.seek(0)  # Reset cursor so we can read it again.
...
class RunJobManagementCommandTest(CeleryTestCase):
    ...
    def test_runjob_db_change_commit_and_username(self):
        ...
        with self.settings(JOBS_ROOT=os.path.join(settings.BASE_DIR, "extras/tests/example_jobs")):
            get_user_model().objects.create(username="example_user")

            out, err = self.run_command("--commit", "--username", "example_user", "local/test_modify_db/TestModifyDB")
            self.assertIn("Running local/test_modify_db/TestModifyDB...", out)
