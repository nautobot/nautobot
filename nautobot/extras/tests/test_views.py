...
class ScheduledJobTestCase(
    ...
    def test_non_valid_crontab_syntax(self):
        ...
        ScheduledJob.objects.create(
            ...
            user=self.user,
            start_time=datetime.now(),
            crontab="with enough values to unpack",
        )
        ...
        response = self.client.get(self._get_url("list"))
        self.assertHttpStatus(response, 200)
        self.assertNotIn("test5", extract_page_body(response.content.decode(response.charset)))
        self.assertNotIn("test6", extract_page_body(response.content.decode(response.charset)))
        self.assertNotIn("test7", extract_page_body(response.content.decode(response.charset)))
        self.assertNotIn("test8", extract_page_body(response.content.decode(response.charset)))
        self.assertNotIn("test9", extract_page_body(response.content.decode(response.charset)))
        self.assertIn("test10", extract_page_body(response.content.decode(response.charset)))
