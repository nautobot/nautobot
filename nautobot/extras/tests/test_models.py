...
class JobResultTestCase(TestCase):
    ...
    def test_is_deletable_property(self):
        """is_deletable should be False for unready results and True once the result is finished."""

        with self.subTest("Not deletable while pending"):
            self.job_result.status = JobResultStatusChoices.STATUS_PENDING
            self.job_result.save()
            self.assertFalse(self.job_result.is_deletable)
        ...
        with self.subTest("Deletable once succeeded"):
            self.job_result.status = JobResultStatusChoices.STATUS_SUCCESS
            self.job_result.save()
            self.assertTrue(self.job_result.is_deletable)
