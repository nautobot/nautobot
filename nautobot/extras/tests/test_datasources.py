...
class GitTest(TestCase):
    ...
    def setUp(self):
        ...
        self.example_request = self.factory.get("/no-op/")
        self.example_request.user = self.user
        # Needed for use with the change_logging decorator
        self.example_request.id = uuid.uuid4()
    ...
    def test_pull_git_repository_and_refresh_data_with_no_data(self, MockGitRepo):
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            with self.settings(GIT_ROOT=tempdir):
                ...
                pull_git_repository_and_refresh_data(self.repo.pk, self.example_request, self.job_result.pk)
