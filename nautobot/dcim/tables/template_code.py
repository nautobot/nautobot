...
<a href="{% url 'dcim:device' pk=record.pk %}">
    {{ record.name|default:'<span class="badge label-info">Unnamed device</span>' }}
</a>
"""
...
{% if record.installed_device_id %}
    {% load helpers %}
    <span class="badge" style="color: {{ record.installed_device.status.color|fgcolor }}; background-color: #{{ record.installed_device.status.color }}">
        {{ record.installed_device.get_status_display }}
    </span>
{% else %}
    <span class="badge label-default">Vacant</span>
{% endif %}
"""
