...
class IPAMInputValidationTests(TestCase):
    ...
    def test_strict_prefix_filters_validate_before_query_construction(self):
        for endpoint in ("prefix", "ipaddress"):
            for value in ("bad-prefix", "192.0.2.1/24", "2001:db8::1/64", str(uuid4())):
                with self.subTest(endpoint=endpoint, value=value):
                    response = self.client.get(
                        reverse(f"ipam-api:{endpoint}-list"), {"prefix_exact": ["192.0.2.0/24", value]}
                    )
                    self.assertEqual(response.status_code, 400, response.content)
                    self.assertIn("prefix_exact", response.data)
        response = self.client.get(reverse("ipam-api:ipaddress-list"), {"prefix_exact": "192.0.2.1"})
        self.assertEqual(response.status_code, 400, response.content)

    def test_strict_prefix_filters_preserve_valid_literals_and_uuid_matches(self):
        for endpoint, expected in (("prefix", self.prefix), ("ipaddress", self.ip)):
            for value in ("192.0.2.0/24", f" {self.prefix.pk} "):
                with self.subTest(endpoint=endpoint, value=value):
                    response = self.client.get(reverse(f"ipam-api:{endpoint}-list"), {"prefix_exact": value})
                    self.assertEqual(response.status_code, 200, response.content)
                    self.assertEqual([obj["id"] for obj in response.data["results"]], [str(expected.pk)])
    ...
    def test_prefix_field_preserves_widget_and_query_cost(self):
        ...
        with self.assertNumQueries(0):
            self.assertEqual(FilterForm().fields["prefix"].clean(["192.0.2.0/24", "  "]), ["192.0.2.0/24"])
        form = FilterForm({"prefix": [str(self.prefix.pk)]})
        with self.assertNumQueries(1):
            self.assertTrue(form.is_valid(), form.errors)
        self.assertEqual(form.cleaned_data["prefix"], ["192.0.2.0/24"])
        rendered = " ".join(str(form["prefix"]).split())
        ...
        with self.assertNumQueries(1), self.assertRaises(ValidationError):
            FilterForm().fields["prefix"].clean([str(uuid4())])
    ...
    def test_available_ips_supports_head_requests(self):
        ...
        # Pylint infers APIRequestFactory.head() rather than APIClient's response.
        self.assertEqual(response.status_code, 200, response.content)  # pylint: disable=no-member
