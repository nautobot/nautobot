...
class IPAMDateMigration0053TestCase(MigratorTestCase):
    migrate_from = ("ipam", "0052_alter_ipaddress_index_together_and_more")
    migrate_to = ("ipam", "0053_fixup_incorrect_parents")

    def prepare(self):
        create_prefixes_and_ips("10.0.0.0/16", apps=self.old_state.apps)

        ContentType = self.old_state.apps.get_model("contenttypes", "ContentType")
        IPAddress = self.old_state.apps.get_model("ipam", "IPAddress")
        ...
        # Start by creating the "intended" hierarchy
        self.root = Prefix.objects.create(
            network="10.0.0.0", broadcast="10.255.255.255", prefix_length=8, ip_version=4, status=self.status
        )
        self.branch1 = Prefix.objects.create(
            network="10.11.0.0",
        ...
        self.leaf11 = Prefix.objects.create(
            network="10.11.12.0",
        ...
        self.leaf12 = Prefix.objects.create(
            network="10.11.13.0",
        ...
        self.branch2 = Prefix.objects.create(
            network="10.12.0.0",
        ...
        self.leaf21 = Prefix.objects.create(
            network="10.12.14.0",
            broadcast="10.12.14.255",
            prefix_length=24,
            ip_version=4,
            status=self.status,
            parent=self.branch2,
        ...
        self.ip111 = IPAddress.objects.create(
            host="10.11.12.1", mask_length=24, ip_version=4, status=self.status, parent=self.leaf11
        )
        self.ip112 = IPAddress.objects.create(
            host="10.11.12.2", mask_length=24, ip_version=4, status=self.status, parent=self.leaf11
        )
        self.ip121 = IPAddress.objects.create(
            host="10.11.13.1", mask_length=24, ip_version=4, status=self.status, parent=self.leaf12
        )
        self.ip122 = IPAddress.objects.create(
            host="10.11.13.2", mask_length=24, ip_version=4, status=self.status, parent=self.leaf12
        )
        self.ip21 = IPAddress.objects.create(
            host="10.12.13.1", mask_length=24, ip_version=4, status=self.status, parent=self.branch2
        )
        self.ip211 = IPAddress.objects.create(
            host="10.12.14.1", mask_length=24, ip_version=4, status=self.status, parent=self.leaf21
        )
        self.ip212 = IPAddress.objects.create(
            host="10.12.14.2", mask_length=24, ip_version=4, status=self.status, parent=self.leaf21
        )

        # Now mess it up in plausible ways

        # Change leaf11 prefix, keeping ip121 and ip122 as its children and claiming ip21
        self.leaf11.network = "10.12.13.0"
        self.leaf11.broadcast = "10.12.13.255"
        self.leaf11.save()

        # Create a new leaf13 prefix but keep its parent as null
        self.leaf13 = Prefix.objects.create(
            network="10.11.14.0",
        ...
        # Create a new ip213 IP but keep its parent as null
        self.ip213 = IPAddress.objects.create(
            host="10.12.14.3", mask_length=24, ip_version=4, status=self.status, parent=None
        )

        # Cross-namespace parentage
        self.other_namespace = Namespace.objects.create(name="Other Namespace")
        ...
        # Prefix with valid but not optimal parent
        self.leaf14 = Prefix.objects.create(
            network="10.11.15.0",
        ...
        # IP with valid but not optimal parent
        self.ip214 = IPAddress.objects.create(
            host="10.12.14.4", mask_length=24, ip_version=4, status=self.status, parent=self.root
        )
